...
本仓库维护同一份规则的两个镜像文件，内容必须保持一致：

```text
AGENTS.md    # 跨工具标准，任何 AI 编程工具都可读取
CLAUDE.md    # Claude Code / Claude app 只自动读取这一份
```

修改任何规则时，**必须同时同步 `AGENTS.md` 与 `CLAUDE.md`**，避免两处漂移。

保留 `CLAUDE.md` 的原因：Claude Code 在会话启动时只自动加载 `CLAUDE.md`，不会自动读取 `AGENTS.md`。缺少 `CLAUDE.md` 时，使用 Claude 编程会丢失本文件中的全部规则。

不要主动创建或复制其他针对某个 AI 编程产品的重复规则文件：
...
或其他重复规则文件，除非用户以后明确要求。

实现每个 Prompt 时，工具应显式读取本文件（其内容与 CLAUDE.md 完全一致）。
